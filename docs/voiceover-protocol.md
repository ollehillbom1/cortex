...
A scripted manual test for one person with one iPhone, about 30 minutes of
actual testing after a 10-minute one-off setup. It exists because CI cannot
do this job: `@axe-core/playwright` runs Chromium-only
...
| 2   | Round feedback and auto-advance     | 3 min | 1800 ms timer, no live region           |
| 3   | Quit dialog (focus trap, dismissal) | 3 min | Focus behaviour is JS, not markup       |
| 4   | Profile toggles and sliders         | 5 min | 8 hidden checkboxes behind styled spans |
| 5   | N-Back (timed visuospatial)         | 5 min | Known-weak; needs measurement, not news |
| 6   | Tab bar, landmarks, Today           | 4 min | Cheap, and one suspected markup bug     |
| 7   | Stats charts (SVG)                  | 3 min | Chart updates may be silent             |
| 8   | Onboarding and profile picker       | 5 min | Stretch; needs a second profile         |

Scenarios 1-3 run as one continuous session, and they populate the history
that scenario 7 needs. Do not reorder those three.
...
- If a profile already exists, go to **Profile → Preferences** and confirm:
  **Sound = on**, **Skip exercises that need sight = off**, **Larger text =
  off**, **Kid mode = off**. Scenarios 1 and 5 assume these.
- A brand-new profile starts every exercise at level 1. All timings quoted
  below are level-1 timings.

### S4. Onboarding, if you see it (record now, audit later in scenario 8)
...
- [ ] **S4b** On the profile form, swipe to the name field and the avatar
      choices.
      _Should:_ "Your name, text field"; the avatars announce as radio
      buttons with a selected state ("bear, selected, radio button, 1 of 8"
      or similar).
      _Heard:_
...
For timed screens (scenarios 1, 2, 5) record something. Prose written after
...
**Screen curtain is the single most useful tool here.** Scenarios 1, 2 and 5
...
supposed to work by ear alone, this is it. Four rounds, roughly 24 s each.
...
- [ ] **1.6** Find and activate the play control.
      ...
      coming ("4 digits will be spoken.").
      _Heard:_
...
- [ ] **1.9** Enter the digits on the keypad. Find "5" by touch-exploring,
      ...
      told what you now have ("Entered 2 of 4 digits" or the digits
      themselves).
...
## Scenario 2 — Round feedback and auto-advance (3 min)
...
## Scenario 3 — Quit dialog: focus trap and dismissal (3 min)

Still in the same session (you should be on round 3 or 4). The dialog is
...
- [ ] **3.1** Find the end-session control in the header.
...
- [ ] **3.3** Swipe right repeatedly (6-8 times) past the last button.
      _Should:_ you stay inside the dialog and wrap around. You must **not**
      reach the game behind it or the tab bar.
      _Did you escape the dialog? What did you reach?:_
- [ ] **3.4** Touch-explore the top third of the screen, outside the dialog
      panel.
      _Should:_ nothing behind the dialog speaks (`aria-modal="true"`).
      _Heard:_
- [ ] **3.5** Two-finger scrub (Z) to dismiss.
      ...
      finding.)
- [ ] **3.6** Reopen the dialog, then activate **Keep training**.
      _Should:_ focus returns to the "End session" button you came from.
      _Where did the cursor go?:_
- [ ] **3.7** Reopen it once more and activate **End session**.
...
## Scenario 4 — Profile toggles and sliders (5 min)

Eight preferences use a `Toggle` whose real control is a visually hidden
checkbox inside a wrapping `<label>`, with the visible track drawn by
sibling `<span>`s (`src/app/(tabs)/profile/page.tsx:670-698`). This pattern
passes axe-core and still fails in practice when the accessible name lands
on the wrong node or the state change is never announced.

Go to the **Profile** tab.

- [ ] **4.1** Set the rotor to **Form Controls** (two-finger rotate), then
      swipe down repeatedly to walk the controls.
      _Should:_ you reach every toggle and both sliders; each stop has a
      name.
      _How many controls did you reach? Any unnamed ("checkbox" with no
      label) stops?:_
- [ ] **4.2** Set the rotor back to Characters/Words and swipe right to the
      **Sound** toggle.
...
- [ ] **4.4** With Sound off, swipe to the **Volume** slider.
      _Should:_ "Volume, adjustable, 70 percent" _and_ "dimmed" (it is
      disabled while sound is off).
      _Heard:_
- [ ] **4.5** Turn Sound back on. On **Volume**, swipe up and down to change
      the value.
      _Should:_ each change announces the new percentage.
      _Heard:_
- [ ] **4.6** Same on **Daily goal**.
      _Should:_ "Daily goal in minutes, adjustable, 10"; values step by 5.
      _Heard:_
- [ ] **4.7** Toggle **Larger text** on.
...
- [ ] **4.8** Swipe to **Skip exercises that need sight**.
...
- [ ] **4.9** Find the destructive controls (export, import, reset, delete)
...
## Scenario 5 — N-Back, a timed visuospatial exercise (5 min)
...
- [ ] **5.1** Before starting, on the instructions screen: does it tell you
      this exercise needs sight?
      _Heard:_
- [ ] **5.2** Screen curtain on. Start the round. Do not touch anything for
...
- [ ] **5.3** Still blind, try to find the **Match** button by touch and
      press it once.
...
- [ ] **5.4** Swipe right through the grid area.
...
- [ ] **5.5** Screen curtain off. Let the round finish, or quit it.
      _Was quitting mid-round possible with VoiceOver on? (y/n, and how):_
- [ ] **5.6** One sentence for the report: what would make this exercise
...
## Scenario 6 — Tab bar, landmarks and the Today screen (4 min)

- [ ] **6.1** Set the rotor to **Landmarks** and swipe down through the
      Today screen.
...
- [ ] **6.2** Rotor to **Headings**, swipe down.
      _Should:_ a sensible heading outline starting at "Cortex".
      _Heard:_
- [ ] **6.3** Touch each of the four tabs along the bottom.
...
- [ ] **6.4** Swipe to the streak counter at the top right.
      ...
      _Heard:_
- [ ] **6.5** Swipe to the main **Start session** control.
...
- [ ] **6.6** If a backup reminder or insight card is showing, find its
      dismiss control.
...
## Scenario 7 — Stats charts (3 min)

Requires history, which scenarios 1-3 produced. Charts are inline SVG
...
- [ ] **7.1** Go to **Stats** and two-finger swipe up to read the screen.
...
- [ ] **7.2** Find the exercise chooser above the accuracy chart and swipe
      through it.
      _Should:_ nine buttons, with the current one announced as "selected".
      _Heard:_
- [ ] **7.3** Select a different exercise.
...
- [ ] **7.4** Find the training balance section.
...
## Scenario 8 — Onboarding and the profile picker (5 min, stretch)
...
- [ ] **8.1** Profile tab → **Add household profile**, create "VO test 2".
      _Should:_ the creation dialog behaves like scenario 3's dialog —
      announced, focus moved into it, closable.
      _Heard:_
- [ ] **8.2** Fully quit the app (swipe up from the app switcher) and
...
- [ ] **8.3** Touch-explore behind the picker.
      _Should:_ background content is silent.
      _Heard:_
- [ ] **8.4** Two-finger scrub to dismiss.
...
- [ ] **8.5** Pick a profile.
      ...
      _Heard:_
- [ ] **8.6** Delete the extra profile afterwards (Profile → the profile →
